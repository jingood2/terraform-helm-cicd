...
output "argocd_manifest" {
  description = "Configure kubectl: make sure you're logged in with the correct AWS profile and run the following command to update your kubeconfig"
  value       = var.enable_arogcd ? helm_release.arogcd[0].manifest : null
}

output "image_updater_manifest" {
  description = "Configure kubectl: make sure you're logged in with the correct AWS profile and run the following command to update your kubeconfig"
  value       = var.enable_image_updater ? helm_release.image_updater[0].manifest : null
}
...
output "sealed_secrets_manifest" {
  description = "Configure kubectl: make sure you're logged in with the correct AWS profile and run the following command to update your kubeconfig"
  value       = var.enable_sealed_secrets ? helm_release.sealed_secrets[0].manifest : null
}
